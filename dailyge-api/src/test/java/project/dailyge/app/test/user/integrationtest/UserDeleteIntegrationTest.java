...
public class UserDeleteIntegrationTest extends IntegrationTestBase {
    ...
    void whenDeleteExistUserThenShouldBeUserNull() {
    ...
    void whenDeleteExistUserThenShouldBeIllegalArgumentException() {
        UserJpaEntity saveUser = userWriteUseCase.save(UserFixture.createUserJpaEntity());
        userWriteUseCase.delete(saveUser.getId());
    ...
    void whenDeleteNotExistUserThenShouldBeUserNotFoundException() {
        assertThrowsExactly(UserTypeException.from(UserCodeAndMessage.USER_NOT_FOUND).getClass(),
            () -> userWriteUseCase.delete(999L));
    }
}
