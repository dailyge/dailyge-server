...
import static project.dailyge.app.test.user.documentationtest.snippet.UserSnippet.LOGOUT_REQUEST_SNIPPET;
...
class LogoutDocumentationTest extends DatabaseTestBase {
    ...
    void whenLogoutThenStatusCodeShouldBe200_OK() throws JsonProcessingException {
        final UserJpaEntity user = userWriteUseCase.save(createUserJpaEntity());
        final LogoutRequest request = new LogoutRequest(user.getId());

        given(this.specification)
            .filter(document(IDENTIFIER,
                LOGOUT_REQUEST_SNIPPET,
